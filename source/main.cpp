...
void register_wndclass() {
    ...
    wc.hIconSm = HICON(LoadImage(hInst, MAKEINTRESOURCE(IDI_APP), IMAGE_ICON, cx, cx, 0));
    THROW_LAST_ERROR_IF(!RegisterClassExW(&wc));
}

void make_window() {
    THROW_LAST_ERROR_IF_NULL(hWnd = CreateWindowW(wnd_class, app_title, 0, 0, 0, 0, 0, HWND_MESSAGE, nullptr, hInst, nullptr));
}
...
void init_tray() {
    ...
    int cx = GetSystemMetrics(SM_CXSMICON);
    ncd.hIcon = HICON(LoadImage(hInst, MAKEINTRESOURCE(IDI_APP), IMAGE_ICON, cx, cx, 0));
    ncd.uVersion = NOTIFYICON_VERSION_4;
    ncd.uCallbackMessage = UM_TRAY;
    THROW_IF_FAILED(StringCchCopyW(ncd.szTip, sizeof(ncd.szTip) / sizeof(WCHAR), app_title));
...
void notify(UINT title_id, UINT info_id) {
    ...
    LoadStringW(hInst, title_id, ncd.szInfoTitle, sizeof(ncd.szInfoTitle) / sizeof(WCHAR));
...
void show_menu() {
    ...
    for (auto wnd : wnds) {
        ...
        THROW_IF_WIN32_BOOL_FALSE(AppendMenuW(menu.get(), is_window_topmost(wnd) ? MF_CHECKED : 0, id, wnd_text));
    ...
    THROW_IF_WIN32_BOOL_FALSE(AppendMenuW(menu.get(), MF_SEPARATOR, id + 1, nullptr));
    ...
    int cmd = TrackPopupMenu(menu.get(), TPM_BOTTOMALIGN | TPM_NONOTIFY | TPM_RETURNCMD, pt.x, pt.y, 0, hWnd, nullptr);
    if (cmd == id) {
        DestroyWindow(hWnd);
    } else if (cmd != 0) {
...
void toggle_top(HWND wnd) {
    if (!SetWindowPos(wnd, is_window_topmost(wnd) ? HWND_NOTOPMOST : HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE)) {
...
bool is_app_window(HWND wnd) {
    ...
    return IsWindowVisible(wnd) &&
        ...
        (!GetWindow(wnd, GW_OWNER) && !(ex_sty & WS_EX_NOACTIVATE) && !(ex_sty & WS_EX_TOOLWINDOW) && GetWindowTextLengthW(wnd) && wcscmp(wnd_class, L"Windows.UI.Core.CoreWindow")));
}
...
HBITMAP get_uwp_icon(HWND wnd) {
    ...
    wil::unique_handle p{ OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid) };
    THROW_LAST_ERROR_IF_NULL(p);
    WCHAR exename[MAX_LOADSTR];
    DWORD exelen = MAX_LOADSTR;
    THROW_IF_WIN32_BOOL_FALSE(QueryFullProcessImageNameW(p.get(), 0, exename, &exelen));
    if (_wcsicmp(exename, L"C:\\Windows\\System32\\ApplicationFrameHost.exe") != 0) {
        return nullptr;
    }
    DWORD real_pid = pid;
...
HBITMAP create_dib(LONG width, LONG height, void** pp_bits=nullptr) {
    ...
    return CreateDIBSection(nullptr, (BITMAPINFO*)&header, DIB_RGB_COLORS, pp_bits, nullptr, 0);
}
...
HBITMAP icon_to_bitmap(HICON icon) {
    ...
    THROW_IF_WIN32_BOOL_FALSE(DrawIconEx(hMemDC.get(), 0, 0, icon, cx, cx, 0, nullptr, DI_NORMAL));
...
HBITMAP load_img(const WCHAR* path, int w, int h) {
    ...
    THROW_IF_FAILED(CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER, __uuidof(IWICImagingFactory), (void**)&factory));
    wil::com_ptr<IWICBitmapDecoder> decoder;
    THROW_IF_FAILED(factory->CreateDecoderFromFilename(found_path, nullptr, GENERIC_READ, WICDecodeMetadataCacheOnLoad, &decoder));
    wil::com_ptr<IWICBitmapFrameDecode> frame;
    THROW_IF_FAILED(decoder->GetFrame(0, &frame));
    wil::com_ptr<IWICFormatConverter> converter;
    THROW_IF_FAILED(factory->CreateFormatConverter(&converter));
    THROW_IF_FAILED(converter->Initialize(frame.get(), GUID_WICPixelFormat32bppBGRA, WICBitmapDitherTypeSpiral4x4, nullptr, 0, WICBitmapPaletteTypeMedianCut));
    wil::com_ptr<IWICBitmapScaler> scaled;
    THROW_IF_FAILED(factory->CreateBitmapScaler(&scaled));
    THROW_IF_FAILED(scaled->Initialize(converter.get(), w, h, WICBitmapInterpolationModeHighQualityCubic));
    void* p_bits;
    wil::unique_hbitmap bitmap{ create_dib(w, -h, &p_bits) };
    THROW_IF_FAILED(scaled->CopyPixels(nullptr, w * 4, w * h * 4, (BYTE*)p_bits));
